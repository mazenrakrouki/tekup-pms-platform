\chapter{Release 1: Authentication and Access Control}

\section*{Introduction}
\addcontentsline{toc}{section}{Introduction}
This chapter presents the first release of PMS. Its job is to make the application usable and
secure before any business feature is built on top of it. It opens with the preparation phase, six
weeks of study that came before the first line of production code. Two sprints follow. Sprint~1
builds the technical foundation and the authentication chain. Sprint~2 delivers the dynamic
authorization model, together with the management of users and resources.

By the end of the release, the platform can authenticate its users and apply a security policy
that stays configurable. It also holds the reference data the next release needs: users, resources
and cost rates.
\section{Preparation phase}
The release opens with six weeks that produced no production code. They ran from 5~January to
13~February and are listed as such in the schedule of Chapter~2
(Table~\ref{tab:schedule}). Their purpose was to arrive at the first sprint already knowing what
had to be built and on which architecture.
It is reported outside the sprint template because it produced no increment: its output is
the analysis, the backlog and the architecture of the three preceding chapters.

\subsection{What the phase produced}
Four deliverables came out of it, each of which is presented in full elsewhere in this report and
only summarised here.

\begin{itemize}
  \item \textbf{The reading of the existing instrument.} The project review workbook was opened
        sheet by sheet, its formulas traced, and its business rules written down. This is the
        analysis of Chapter~1, and it is the reason the platform reproduces the company's financial
        model rather than an approximation of it.
  \item \textbf{The requirements and the product backlog.} The rules extracted from the workbook
        were turned into actors, functional and non-functional requirements, and thirty-two user
        stories distributed across four releases. This is Chapter~2.
  \item \textbf{The architecture and the technical stack.} The modular monolith, the dynamic
        authorization model, the hybrid indicator engine and the relational schema were decided
        before any of them was written. This is Chapter~3.
  \item \textbf{Design sketches.} A set of throwaway sketches used to picture what a platform of
        this kind might look like. They are the subject of the rest of this section.
\end{itemize}

\subsection{Design sketches}
\label{sec:sketches}
Before writing any interface, a set of sketches was produced with \textbf{Google Stitch}, a
generative design tool published by Google: a screen is described in one or two sentences of plain
language and the tool returns a plausible, complete layout, which can then be refined by describing
what should change.

\begin{figure}[H]
  \centering
  \includegraphics[height=1.6cm]{img/logo_stitch.png}
  \caption{Google Stitch, the tool used to produce the sketches of the preparation phase}
  \label{fig:logo-stitch}
\end{figure}

It was chosen because of what this project did not have: a designer, and time. A conventional
mock-up costs hours before it can be judged, which in practice means producing one arrangement and
committing to it. A tool that returns a full screen in a minute makes it affordable to produce
several arrangements of the same information and throw most of them away, and it is the throwing
away that has the value. It was chosen for exploration, not for production, and it was used
accordingly.

The sketches served one purpose: to make the problem concrete. Reading a backlog of thirty-two
user stories tells you what the platform must do; looking at a screen tells you how much information
a single person is expected to hold at once, and whether four different audiences really need four
different entry points or only one with filters.

\textbf{These sketches are not the application, and nothing in the delivered interface derives from
them.} They were never treated as specifications, never handed to a sprint as an input, and never
implemented. The frontend presented in the rest of this report was designed and built
independently, from the requirements and from the constraints of the authorization model. Any
resemblance between a sketch and a delivered screen is the resemblance any two project-management
interfaces have to each other, not a lineage.

They are shown here because they were a real part of the preparation phase and because they carry
its reasoning: what the sketches made visible, before a line of Angular existed, is that this
platform has four distinct readers rather than one. A director scanning a portfolio, a project
manager working inside a single project, a developer entering their own time, and an administrator
editing the policy are four different information needs, and no single screen serves them all. That
observation survived into the architecture; the pixels did not.

Every figure in this section carries a banner and a watermark naming the tool, so that none of them
can be mistaken for a screenshot of the delivered platform. Two further differences make the
distinction visible in the images themselves: the sketches carry a provisional product name,
\emph{ProjectControl}, and they show amounts in euros. The delivered platform is named PMS and
reports in Tunisian dinars.

\runin{Sign-in and portfolio}
The first sketches addressed the two screens with the widest audience: the entry point, and
the portfolio overview on which a director meets every project at once.

\begin{figure}[H]
  \centering
  \begin{minipage}{0.47\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{img/mk_login.png}
  \end{minipage}\hfill
  \begin{minipage}{0.47\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{img/mk_portfolio.png}
  \end{minipage}
  \caption{Design sketches --- sign-in screen (left) and portfolio overview (right)}
  \label{fig:mk-signin}
\end{figure}

\runin{Sketches of a single project}
Three further sketches explored what one project looks like from the inside. The first takes
it on its financial side; the second resource by resource and month by month; the third
follows the money out, as a list of billing milestones and their status.

\begin{figure}[H]
  \centering
  \begin{minipage}{0.47\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{img/mk_financial.png}
  \end{minipage}\hfill
  \begin{minipage}{0.47\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{img/mk_workload.png}
  \end{minipage}
  \caption{Design sketches --- financial analysis of a project (left) and resource workload matrix (right)}
  \label{fig:mk-project}
\end{figure}

\begin{figure}[H]
  \centering
  \begin{minipage}{0.47\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{img/mk_billing.png}
  \end{minipage}
  \caption{Design sketch --- billing milestones}
  \label{fig:mk-billing}
\end{figure}

\runin{Sketches of the restricted roles}
The last two sketches pictured the two roles that see the least: a developer, and an
administrator who manages accounts and the authorization policy without opening a single
project.

\begin{figure}[H]
  \centering
  \begin{minipage}{0.47\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{img/mk_developer.png}
  \end{minipage}\hfill
  \begin{minipage}{0.47\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{img/mk_admin.png}
  \end{minipage}
  \caption{Design sketches --- developer workspace (left) and administration panel (right)}
  \label{fig:mk-roles}
\end{figure}

\subsection{Closing the phase}
The sketches were set aside once the phase ended. What was carried into Sprint~1 was not a design
but the four conclusions of the preparation phase: the business logic to reproduce, the backlog to
build it from, the architecture to build it on, and the observation that the platform must serve
four distinct readers.

The phase closed with the development environment in place, that is the repository, the database,
the migration tool and the build, so that the first sprint began with a running, empty application
rather than with its installation.

% =====================================================================
\section{Sprint 1 --- Authentication and technical foundation}

\subsection{Sprint objectives}
Sprint~1 ran from 16~February to 6~March, three weeks (Table~\ref{tab:schedule}). Its objective is
to make the platform reachable and the session trustworthy before any business object exists. The
sprint delivers the technical skeleton of the modular monolith and the whole authentication chain,
from sign-in to revocation.

\subsection{Sprint backlog}
The sprint commits the following items of the product backlog (Table~\ref{tab:backlog}).

\begin{longtable}{|m{1.6cm}|m{12.4cm}|}
\caption{Sprint 1 backlog}\label{tab:sb1}\\
\hline
\textbf{Story} & \textbf{User story} \\
\hline
\endfirsthead
\hline
\textbf{Story} & \textbf{User story} \\
\hline
\endhead
\endfoot
\endlastfoot
\hline
US-01 & As a user, I want to sign in with an e-mail and a password \\
\hline
US-02 & As a user, I want to be forced to change an imposed password on first login \\
\hline
US-03 & As an administrator, I want to revoke a user's sessions immediately \\
\hline
\end{longtable}

\subsection{Task planning}
The author being the sole developer, each item was expanded directly into the technical work it
required rather than distributed across a team.

\begin{longtable}{|m{1.6cm}|m{12.4cm}|}
\caption{Sprint 1 task planning}\label{tab:tp1}\\
\hline
\textbf{Story} & \textbf{Technical work} \\
\hline
\endfirsthead
\hline
\textbf{Story} & \textbf{Technical work} \\
\hline
\endhead
\endfoot
\endlastfoot
\hline
US-01 & Authentication service, BCrypt verification, JWT issuance signed with HMAC-SHA512, refresh cookie, token rotation \\
\hline
US-02 & \texttt{first\_login} flag, dedicated filter restricting the token to the password-change endpoint \\
\hline
US-03 & Token version persisted per user, embedded in each token and compared on every request \\
\hline
\end{longtable}

\subsection{Design}
The sprint covers the three use cases that every user must traverse before reaching a business
screen: authenticating, changing an imposed password, and terminating a session. Every other use
case of the platform includes authentication, which is why this sprint precedes all the others.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\columnwidth]{img/uc_sprint1.png}
  \caption{Use case diagram of sprint 1}
  \label{fig:uc-s1}
\end{figure}

\runin{Sequence of the ``Authenticate'' use case}
The nominal scenario involves the browser, the authentication controller, the authentication
service, the token service and the user repository. It shows where the password is verified, where
the two tokens are produced, and why only one of them is written to a cookie.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\columnwidth]{img/seq_s1_login.png}
  \caption{Sequence diagram of the ``Authenticate'' use case}
  \label{fig:seq-s1-login}
\end{figure}

\runin{Sequence of the ``Change the password on first login'' use case}
An account created by the administrator carries a temporary password. The scenario shows how the
token issued at the first connection is restricted, how any business call is refused while the flag
remains set, and how a full token is issued once the password has been changed.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\columnwidth]{img/seq_s1_first_login.png}
  \caption{Sequence diagram of the ``Change the password on first login'' use case}
  \label{fig:seq-s1-first}
\end{figure}

\runin{Sequence of the ``Revoke a session'' use case}
Revocation is the scenario that justifies the design of the token. It shows the increment of the
version, the invalidation of the cache entry, and the rejection of a request presenting a token
issued before the increment.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\columnwidth]{img/seq_s1_revoke.png}
  \caption{Sequence diagram of the ``Revoke a session'' use case}
  \label{fig:seq-s1-revoke}
\end{figure}

\runin{Design decisions}
Three decisions taken in this sprint constrain the whole platform and are therefore justified here
rather than in the realisation.

\textbf{A modular monolith rather than services.} The code is organised into independent functional
packages, each with its own Controller $\rightarrow$ Service $\rightarrow$ Repository $\rightarrow$
Domain layering. A single deployable artefact keeps transactions local and avoids distributed
consistency, which matters because the financial computations of release~3 read from several
modules within one transaction. The package boundaries preserve the option of extraction later.

\textbf{Two tokens rather than one.} A short-lived access token held in memory is paired with a
refresh token confined to an \texttt{HttpOnly} cookie. A single long-lived token stored in the
browser would be readable by any injected script; splitting them means the credential able to mint
new sessions is never reachable from JavaScript. The refresh token is additionally rotated on every
use, so a stolen cookie stops working as soon as the legitimate holder refreshes.

\textbf{Revocation by version rather than by blacklist.} A JWT is self-contained and cannot be
withdrawn once issued. A server-side blacklist would reintroduce the shared state that the stateless
design avoids. Storing a single integer per user and embedding it in each token achieves the same
result: incrementing it invalidates every session of that identity at once, at the cost of one
integer comparison per request.

\subsection{Implementation}
The sprint produced two things in this order: the skeleton every later module is written inside,
and the authentication chain that runs in it.


\runin{Modular monolith structure}
The modules are \texttt{auth}, \texttt{user}, \texttt{project}, \texttt{team}, \texttt{workload},
\texttt{agile}, \texttt{kpi}, \texttt{billing}, \texttt{mission}, \texttt{governance} and
\texttt{shared}. Each subsequent sprint adds its module without modifying the previous ones.

\runin{Base entity and automatic auditing}
All business entities inherit from \texttt{BaseEntity}, which centralises the auto-generated primary
key, the \texttt{createdAt} and \texttt{updatedAt} timestamps managed by \texttt{@PrePersist} and
\texttt{@PreUpdate}, and the \texttt{deleted} marker implementing logical deletion. No data is ever
physically erased, and every repository implicitly filters on \texttt{deleted = false}. This
satisfies the auditability requirement (NFR-008).

\runin{Schema management through Flyway}
The database structure is managed exclusively by Flyway. Hibernate never emits implicit DDL: the
property \texttt{spring.jpa.\allowbreak hibernate.\allowbreak ddl-auto} is set to \texttt{validate},
so a divergence between the entities and the schema stops the application at startup rather than
corrupting data at runtime. The versioned migrations are idempotent and constitute the living
documentation of the schema.

\runin{Token issuance and rotation}
On \texttt{POST /api/auth/login} the authentication service verifies the e-mail, compares the
supplied password with the BCrypt hash, checks that the account is active, then issues a short-lived
JWT access token signed with \texttt{HMAC-SHA512} carrying the e-mail, the token version and the
list of permission codes, together with a refresh token placed in the protected cookie.

\texttt{POST /api/auth/refresh} exchanges a valid cookie for a new access token and a new refresh
cookie; the previous one is rejected from that moment on.

\runin{Immediate revocation by token version}
Revocation rests on a \texttt{token\_version} field persisted in the \texttt{users} table and
embedded in every issued token. On each request the \texttt{JwtAuthenticationFilter} compares the
version carried by the token with the value read from an in-memory cache; if they differ the request
is rejected with \texttt{401 Unauthorized}. Incrementing the field therefore closes every active
session of that identity instantly, which is what happens on logout and on password change, the
latter also terminating sessions opened on other devices.

\runin{Protection against repeated failures}
\texttt{LoginAttemptTracker} counts consecutive failures per identity and temporarily refuses
further attempts, answering \texttt{429 Too Many Requests}. Password guessing is therefore bounded
without locking a legitimate account permanently.

\runin{Forced password change on first login}
When an account is created the administrator sets a temporary password and \texttt{first\_login} is
set to \texttt{true}. The token issued at the first connection carries a restricted right enforced
by \texttt{FirstLoginFilter}: only \texttt{POST /api/\allowbreak auth/\allowbreak change-password}
is authorised. Once the password is changed, \texttt{first\_login} becomes \texttt{false} and an
unrestricted token is issued.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\columnwidth]{img/ui_login.png}
  \caption{Login interface}
  \label{fig:ui-login}
\end{figure}

\emph{\small Note on the screenshots. Every screen reproduced in this chapter and the following
ones was captured against a demonstration dataset generated for the purpose. The project codes,
client names, staff names and amounts they show are fictitious and correspond to no real engagement
of the host organisation. No production data appears in this document.}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\columnwidth]{img/ui_change_password.png}
  \caption{Forced password change interface}
  \label{fig:ui-change-password}
\end{figure}

\subsection{Testing}
The tests for this sprint sit with the module they check, and they run together with all the
others. They were run again for this report, and the counts below come from that run.

\textbf{\texttt{AuthControllerTest} (9 tests).} It signs in with the right password, then with a
wrong one. It calls the refresh endpoint with the cookie and without it. After a rotation, it
checks that the old cookie no longer works. It ends on the context endpoint, called once with a
token and once without.

All 9 of them pass.

\subsection{Sprint review}
The sprint was accepted once three things worked in front of the supervisor. An account could sign
in. A new account was forced to change its password on the first login. A session could be cut off
at once. None of the later sprints can be shown before this one works, and that is why the release
starts here.

% =====================================================================
\section{Sprint 2 --- Dynamic access control, users and resources}

\subsection{Sprint objectives}
Sprint~2 ran from 9 to 27~March, three weeks (Table~\ref{tab:schedule}). Its objective is to put
the authorization policy in the database rather than in the code, and load the reference data the
operational modules consume. This is the decision the rest of the platform rests on.

\subsection{Sprint backlog}
The sprint commits the following items of the product backlog (Table~\ref{tab:backlog}).

\begin{longtable}{|m{1.6cm}|m{12.4cm}|}
\caption{Sprint 2 backlog}\label{tab:sb2}\\
\hline
\textbf{Story} & \textbf{User story} \\
\hline
\endfirsthead
\hline
\textbf{Story} & \textbf{User story} \\
\hline
\endhead
\endfoot
\endlastfoot
\hline
US-04 & As an administrator, I want to assign permissions to roles without redeploying \\
\hline
US-05 & As a user, I want to see only the menu entries my account is entitled to \\
\hline
US-06 & As an administrator, I want to create a user account \\
\hline
US-07 & As an administrator, I want to modify a user account \\
\hline
US-08 & As an administrator, I want to deactivate a user account \\
\hline
US-09 & As an administrator, I want to record resources and their yearly cost rate \\
\hline
\end{longtable}

\subsection{Task planning}
The author being the sole developer, each item was expanded directly into the technical work it
required rather than distributed across a team.

\begin{longtable}{|m{1.6cm}|m{12.4cm}|}
\caption{Sprint 2 task planning}\label{tab:tp2}\\
\hline
\textbf{Story} & \textbf{Technical work} \\
\hline
\endfirsthead
\hline
\textbf{Story} & \textbf{Technical work} \\
\hline
\endhead
\endfoot
\endlastfoot
\hline
US-04 & \texttt{roles}, \texttt{permissions} and \texttt{role\_permissions} tables, permissions carried in the token and converted into Spring Security authorities \\
\hline
US-05 & Context endpoint returning the held permissions, navigation built from that response \\
\hline
US-06 & User module, creation by the administrator with a generated temporary password \\
\hline
US-07 & Update endpoint preserving e-mail uniqueness, role reassignment \\
\hline
US-08 & Activation state on the account: deactivation rather than deletion, history preserved \\
\hline
US-09 & Resource entity separate from the account, loaded cost rate stored per resource and per year \\
\hline
\end{longtable}

\subsection{Design}
The sprint covers the administration use cases: assigning permissions to a role, managing user
accounts, and recording resources with their yearly cost rate. All of them are reserved to the
administrator, and all of them include the authentication delivered by sprint~1.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\columnwidth]{img/uc_sprint2.png}
  \caption{Use case diagram of sprint 2}
  \label{fig:uc-s2}
\end{figure}

\runin{Model of the authorization policy}
The authorization model is the structuring design of the platform and deserves its own class
diagram. A role aggregates permissions, a permission belongs to a functional module, and a user
holds one active role. Nothing in this model is compiled into the application: the association
between a role and its permissions is a table, so the policy is data.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\columnwidth]{img/class_rbac.png}
  \caption{Class diagram of the authorization model}
  \label{fig:class-rbac}
\end{figure}

\runin{Sequence of the ``Grant a permission to a role'' use case}
This scenario is the proof that the policy is configurable. It shows the administrator adding a
permission to a role, the association being persisted, and the affected users obtaining the new
right on their next token without any redeployment.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\columnwidth]{img/seq_s2_grant_permission.png}
  \caption{Sequence diagram of the ``Grant a permission to a role'' use case}
  \label{fig:seq-s2-grant}
\end{figure}

\runin{Design decisions}
\textbf{Permissions rather than roles in the code.} No role name is ever tested in the application.
Only permission codes are, through annotations of the form

\begin{center}
\texttt{@PreAuthorize("hasAuthority('CREATE\_PROJECT')")}
\end{center}

\noindent
Testing a role would hard-code the policy into the source and make every change a release; testing
a permission leaves the policy in the database.

\textbf{A resource is not an account.} A resource represents a cost-bearing capacity and may exist
without a platform account, since the workbook records people who never connect to any system.
Merging the two notions would have made it impossible to cost a contributor who has no login.

\textbf{A cost rate per year, not per resource.} The loaded cost rate is recorded per resource
\emph{and per year}, so that the cost of a man-day uses the rate of the year in which the effort was
booked. This requirement comes directly from the workbook, where the 2024 and 2025 rates of the same
person differ.

\subsection{Implementation}
The policy model comes first, then the two places it is read, the security layer on the server and
the navigation on the client, and last the data the administrator maintains.


\runin{Permission, role and user model}
The \texttt{role\_permissions} table associates a set of permissions with a role. A user holds a
single active role, but that role can be reconfigured at any time by the administrator without
redeployment. A dedicated migration initialises the four default roles, Administrator, Director,
Project Manager and Developer, with eighteen permission codes spread over seven functional modules.

\runin{Propagation to Spring Security}
The \texttt{JwtAuthenticationFilter} reads the permission codes from the token and converts them
into \texttt{SimpleGrantedAuthority} objects, after which every REST entry point is protected by its
\texttt{@PreAuthorize} annotation.

\runin{Dynamic menu}
The context endpoint returns, for the authenticated user, the list of their permissions together
with their profile information. The Angular frontend builds the navigation menu from that response:
an item is displayed only if the user holds the required permission code. The interface therefore
reflects the authorization policy automatically, with no duplicated rule on the client side.

Figures~\ref{fig:ui-perm-catalogue} to~\ref{fig:ui-dev-dashboard} show the effect on the same
build of the application, signed in as three different roles. The permission catalogue states the
policy; the two dashboards show what it produces.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\columnwidth]{img/ui_permissions.png}
  \caption[Permission catalogue]{Permission catalogue: the policy as the administrator reads it, grouped by functional
           module, with the roles that hold each code}
  \label{fig:ui-perm-catalogue}
\end{figure}

The first session is a project manager's.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\columnwidth]{img/ui_pm_dashboard.png}
  \caption[Project manager view]{Project manager view: the menu carries the finance and governance sections, and the
           workspace is limited to the projects the user manages}
  \label{fig:ui-pm-dashboard}
\end{figure}

The second is a developer's, on the same deployment and the same data.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\columnwidth]{img/ui_dev_dashboard.png}
  \caption[Developer view of the same application]{Developer view of the same application: no cost rates, no indicators, no billing and no
           governance, because the role holds none of those permission codes}
  \label{fig:ui-dev-dashboard}
\end{figure}

The comparison is the point of the sprint. Nothing distinguishes these two sessions except the
row that links their account to a role, and the rows that link that role to permissions. No branch
in the code names a role, and no menu is hidden by a hard-coded condition: removing
\texttt{VIEW\_KPI} from the developer role removes the entry, and granting it back restores it, in
both cases without recompiling anything. The requirement that a developer must never see the margin
of the project they work on is met by the absence of a row, not by an \texttt{if}.

\runin{Users and resources}
The \texttt{user} module manages accounts, that is creation by the administrator with a temporary
password, modification, activation and deactivation, and role assignment, together with the human
resources and their daily and loaded cost rates. Conversions between entities and data transfer
objects are handled by MapStruct mappers.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\columnwidth]{img/ui_users.png}
  \caption{User management interface}
  \label{fig:ui-users}
\end{figure}

The same module carries the screen on which roles are given their permissions.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\columnwidth]{img/ui_roles.png}
  \caption{Role and permission management interface}
  \label{fig:ui-roles}
\end{figure}

Resources are kept apart from accounts, each carrying the yearly cost rate the indicators are
valued at.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\columnwidth]{img/ui_resources.png}
  \caption{Resource and yearly cost rate interface}
  \label{fig:ui-resources}
\end{figure}

\subsection{Testing}
The tests for this sprint sit with the module they check, and they run together with all the
others. They were run again for this report, and the counts below come from that run.

\textbf{\texttt{ProjectScopeServiceTest} (8 tests).} A user who holds the global permission
reaches every project. A user without it reaches only the projects in their own scope. If that
user asks for a project outside the scope, the call is refused. It does not simply come back
empty.

All 8 of them pass.

\subsection{Sprint review}
One check decided the review. A permission was removed from a role. The matching entry disappeared
from the menu, and the matching call was refused by the API. The permission was granted back, and
both returned. Nothing was recompiled or redeployed between the two.

% =====================================================================
\section*{Conclusion}
\addcontentsline{toc}{section}{Conclusion}
Release~1 built the foundation the whole platform rests on. It delivered a modular skeleton with
auditing and controlled migrations, and an authentication chain that rotates its tokens and can
close a session at once. Above all it delivered a dynamic authorization model, which keeps the
security policy in the database instead of in the code. The platform now knows who the user is and
what that user may do, and it holds the reference data the business modules need. The next release
uses this foundation to manage projects and workload.
