...
\endlastfoot
\hline
US-15 & Define billing milestones following contractual progress & Milestone as a percentage of the
effective budget, sum capped at 100\% in the service layer & 6 \\
\hline
US-16 & Record payments received against a milestone & Payments kept as individual rows so partial
settlements stay visible & 6 \\
\hline
US-17 & Record amendments so the revised budget reflects contractual change & Amendment as the
single writer of the revised budget & 6 \\
\hline
US-18 & Include travel costs in the project cost & Mission with typed components, each converted at
the mission date before summation & 6 \\
\hline
US-19 & Keep a risk register & Risk with probability, impact, status and mitigation plan & 7 \\
\hline
US-20 & Track deliverables and measure delivery progress & Deliverable with a controlled status,
feeding the delivery rate of the indicator engine & 7 \\
\hline
US-21 & Document stakeholders and change requests & Two registers composed by the project, edited by
the project manager & 7 \\
\hline
US-22 & Give the project an economic baseline & Internal quote structured by section, gated by a
dedicated permission & 8 \\
\hline
US-23 & Compute the financial indicators automatically & Recompute-on-read engine valuing effort at
the cost rate of its own year & 8 \\
\hline
US-24 & Freeze a monthly review as history & Immutable snapshot storing the indicators and the
earned value entered by the manager & 8 \\
\hline
US-25 & Supervise the whole portfolio & Per-project and portfolio dashboards derived from the same
engine & 8 \\
\hline
\caption{Backlog of release 3}
\label{tab:backlog-r3}
\end{longtable}
...
% TODO: render docs/uml/sprint/uc_sprint6.puml then uncomment.
% \begin{figure}[H]
%   \centering
...
% TODO: render docs/uml/sprint/uc_sprint7.puml then uncomment.
% \begin{figure}[H]
%   \centering
...
% TODO: render docs/uml/sprint/uc_sprint8.puml then uncomment.
% \begin{figure}[H]
%   \centering
